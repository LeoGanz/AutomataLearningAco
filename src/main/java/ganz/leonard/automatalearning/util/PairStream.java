...
/**
 * Helper for working with streams of maps. <br>
 * From https://stackoverflow.com/a/29254246
 *
 * @author Holger on StackOverflow https://stackoverflow.com/users/2711488/holger
 */
public interface PairStream<K, V> {
  ...
  default <RK, RV> PairStream<RK, RV> flatMap(
    ...
    return from(entries().flatMap(e -> mapper.apply(e.getKey(), e.getValue()).entries()));
  }
